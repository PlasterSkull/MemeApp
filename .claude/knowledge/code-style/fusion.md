...
@inject IMemeService MemeService

@code {
    [Parameter] public string Tag { get; set; } = "";

    protected override async Task<ImmutableList<MemeDto>> ComputeState(CancellationToken cancellationToken)
        => await MemeService.ListByTagAsync(Tag, cancellationToken);
}
```

In markup use `State.Value`, `State.HasValue`, `State.Error`:
...
{
    foreach (var meme in State.Value)
    {
        <MemeCard Meme="meme" />
    }
}
...
protected override async Task<ImmutableList<MemeDto>> ComputeState(CancellationToken cancellationToken)
{
    ...
    {
        return ImmutableList<MemeDto>.Empty;
    }
    return await MemeService.GetMyMemesAsync(Session, cancellationToken);
}
```
